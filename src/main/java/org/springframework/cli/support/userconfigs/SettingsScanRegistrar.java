...
/**
 * {@link ImportBeanDefinitionRegistrar} for {@link SettingsScan @UserConfigsScan}.
 *
 * @author Janne Valkealahti
 */
class SettingsScanRegistrar implements ImportBeanDefinitionRegistrar {

	private final Environment environment;
	private final ResourceLoader resourceLoader;

	SettingsScanRegistrar(Environment environment, ResourceLoader resourceLoader) {
	...
	private Set<String> getPackagesToScan(AnnotationMetadata metadata) {
		AnnotationAttributes attributes = AnnotationAttributes
				.fromMap(metadata.getAnnotationAttributes(SettingsScan.class.getName()));
	...
	private void scan(BeanDefinitionRegistry registry, Set<String> packages) {
		...
		for (String basePackage : packages) {
			for (BeanDefinition candidate : scanner.findCandidateComponents(basePackage)) {
				try {
					Class<?> type = ClassUtils.forName(candidate.getBeanClassName(), null);
					types.add(type);
		...
		if (!types.isEmpty()) {
			...
			RootBeanDefinition beanDefinition = new RootBeanDefinition(SettingsHolder.class, cav, null);
